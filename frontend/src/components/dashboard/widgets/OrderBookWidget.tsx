                    ...
                    <div className="flex-1 flex flex-col justify-end overflow-hidden">
                        {asks.map(([price, qty], i) => (
                            <div key={`ask-${i}`} className="grid grid-cols-2 gap-2 px-2 py-0.5 hover:bg-red-500/5 relative group">
                                <span className="text-red-400 z-10">{parseFloat(price).toFixed(2)}</span>
                                <span className="text-muted-foreground text-right z-10">{parseFloat(qty).toFixed(4)}</span>
                                <div className="absolute right-0 top-0 bottom-0 bg-red-500/10 transition-all duration-200" style={{ width: `${Math.min(parseFloat(qty) * 100, 100)}%` }} />
                            </div>
                        ))}
                    </div>

                    <div className="border-t border-b border-white/10 my-1 py-1 text-center text-white font-bold bg-white/5">
                        {orderBook ? parseFloat(orderBook.a[0][0]).toFixed(2) : '---'}
                    </div>
                    ...
                    <div className="flex-1 overflow-hidden">
                        {bids.map(([price, qty], i) => (
                            <div key={`bid-${i}`} className="grid grid-cols-2 gap-2 px-2 py-0.5 hover:bg-green-500/5 relative group">
                                <span className="text-green-400 z-10">{parseFloat(price).toFixed(2)}</span>
                                <span className="text-muted-foreground text-right z-10">{parseFloat(qty).toFixed(4)}</span>
                                <div className="absolute right-0 top-0 bottom-0 bg-green-500/10 transition-all duration-200" style={{ width: `${Math.min(parseFloat(qty) * 100, 100)}%` }} />
                            </div>
                        ))}
                    </div>
                </div>
